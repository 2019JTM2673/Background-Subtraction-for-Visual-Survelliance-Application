...
init_frame = np.zeros([height, width])
...
while(videoread.isOpened()):
    ...
    img2 = cv2.cvtColor(img1,cv2.COLOR_BGR2GRAY)      ############## Converting each frame into gray scale ##################
...
#################################################  Start manipulation for each pixel of video frame ########################################
    for i in range (row):
        for j in range (col):
            ...
            k = 0
            pix = img2[i][j]
            while(sum<0.4 and k < 4):
                sum = sum + new_wt[k]
                k+=1
            # first k indices are the background gaussians
            new_loca = new_search(pix)
